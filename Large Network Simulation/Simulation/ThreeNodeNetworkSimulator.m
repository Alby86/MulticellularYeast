...
SpN=length(IAA)*length(alpha)*length(beta)*N*(N-1)*(N-2)/6;
SpNSS=length(IAA)*length(alpha)*N*(N-1)*(N-2)/6;
Y1=zeros(SpN,TS+1);
Y2=zeros(SpN,TS+1);
Y3=zeros(SpN,TS+1);
Y4=zeros(SpN,TS+1);
...
for i=1:N
    ...
    for j=i+1:N
        ...
        cont3=cont3+1;
        input0=[0,0,0];
        parS=[par(i,:),par(j,:),par(kk,:)];
        inputS0=[inputT,inputT,inputT];
        outputS0=[outputT,outputT,outputT];
        K0=ones(4,1);
        [~,yODE2]=ode15s(@ThreeStrainSimulatorRatio,[0 Tt],x0,odeset('refine',10),input0,parS,inputS0,outputS0,K0,Trans);
        X0=yODE2(end,:);
        X0(3)=0; 
        X0(6)=0;
        x0(31)=0;
        ...
        for k=1:length(IAA)
            for q=1:length(alpha)
                ...
                for w=1:length(beta)
                    ...
                    inputS=[input(i,:),input(j,:),input(kk,:)];
                    outputS=[output(i,:),output(j,:),output(kk,:)];
                    [t,yODE]=ode15s(@ThreeStrainSimulatorRatio,[0 TS],X0,odeset('refine',10),INPUTS,parS,inputS,outputS,K0,Sim);
                    ...
                    Yindex(cont,:)=[i,j,kk,IAA(k),alpha(q),beta(w)]; %Index for the time series
                    YSSindex(cont2,:)=[i,j,kk,IAA(k),alpha(q)];
                    Ylabel={'Strain #1','Strain #2','IAA (nM)','alpha (nM)','beta (nM)'}; 
                    
                    if k==1 && q==1 && w==1 %no input
                        GATElabel(cont3,1:3)=[i,j,kk];
...
filename='ThreeNodeNetworkSimulation';
save(filename)