%Code to simulate any 2 random strain combination from the library
...
SpN=length(IAA)*length(alpha)*length(beta)*N*(N-1)/2;
SpNSS=length(IAA)*length(alpha)*N*(N-1)/2;
Y1=zeros(SpN,TS+1);
Y2=zeros(SpN,TS+1);
Y3=zeros(SpN,TS+1);
Y4=zeros(SpN,TS+1);
...
for i=1:N
    ...
    for j=i+1:N
        ...
        input0=[0,0,0];
        parS=[par(i,:),par(j,:)];
        inputS0=[inputT,inputT];
        outputS0=[outputT,outputT];
        K0=ones(2,1);
        [~,yODE2]=ode15s(@TwoStrainSimulatorRatio,[0 Tt],x0,odeset('refine',10),input0,parS,inputS0,outputS0,K0,Trans);
        X0=yODE2(end,:);
        X0(3)=0; 
        X0(6)=0;
        ...
        for k=1:length(IAA)
            for q=1:length(alpha)
                ...
                for w=1:length(beta)
                    ...
                    inputS=[input(i,:),input(j,:)];
                    outputS=[output(i,:),output(j,:)];
                    [t,yODE]=ode15s(@TwoStrainSimulatorRatio,[0 TS],X0,odeset('refine',10),INPUTS,parS,inputS,outputS,K0,Sim);
                    ...
                    Yindex(cont,:)=[i,j,IAA(k),alpha(q),beta(w)]; %Index for the time series
                    YSSindex(cont2,:)=[i,j,IAA(k),alpha(q)];
                    Ylabel={'Strain #1','Strain #2','IAA (nM)','alpha (nM)','beta (nM)'}; 
                    
                    if k==1 && q==1 && w==1 %no input
                        GATElabel(cont3,1:2)=[i,j];
...
filename='TwoNodeNetworkSimulationFixedWithMISOF';
save(filename,'Y1','Y1SS','Y2','Y2SS','Yindex','YSSindex','Ylabel','Y3','Y4','Y5','Y6','Y3SS','Y4SS','Y5SS','Y6SS','GATE','GATElabel')