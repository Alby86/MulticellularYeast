...
function y=FourStrainSimulatorRatio(t,x,inputs,par,input,output,K,T)
...
output1=output(1:5);
output2=output(6:10);
output3=output(11:15);
output4=output(16:20);

%Assigning the ratio parameters and inputs
alpha=(alpha0+K(1)*x(3)*output1(1)+K(2)*x(6)*output2(1)+K(3)*x(31)*output3(1)+K(4)*x(34)*output4(1))/(1+K(1)*x(3)*output1(4)+K(2)*x(6)*output2(4)+K(3)*x(31)*output3(4)+K(4)*x(34)*output4(4)); %updating alpha
IAA=(IAA0+K(1)*x(3)*output1(2)+K(2)*x(6)*output2(2)+K(3)*x(31)*output3(2)+K(4)*x(34)*output4(2))/(1+K(1)*x(3)*output1(5)+K(2)*x(6)*output2(5)+K(3)*x(31)*output3(5)+K(4)*x(34)*output4(5)); %updating IAA

%beta remains the same since it can neither be degraded nor created

%Assigning the correct input to the correct strain
u=alpha*input1(1)+IAA*input1(2)+beta*input1(3);
v=alpha*input2(1)+IAA*input2(2)+beta*input2(3);
w=alpha*input3(1)+IAA*input3(2)+beta*input3(3);
z=alpha*input4(1)+IAA*input4(2)+beta*input4(3);

%Small fix in case of computation errors
x(x<0)=0;

%Assigning the variables for the simulation
if u==0
    x(1)=0;
end
%Strain #1
...
%Strain #3
y(29)=parf3(1)*w-parf3(2)*x(29);
if parf3(end)==1
    y(30)=parf3(3)*x(29)^parf3(5)/(parf3(4)+x(29)^parf3(5))-parf3(6)*x(30);
else
    y(30)=parf3(3)/(parf3(4)+x(29)^parf3(5))-parf3(6)*x(30);
end
y(31)=parf3(9)+parf3(7)*x(30)-parf3(8)*x(31);

%Strain #4
y(32)=parf4(1)*z-parf4(2)*x(32);
if parf4(end)==1
    y(33)=parf4(3)*x(32)^parf4(5)/(parf4(4)+x(32)^parf4(5))-parf4(6)*x(33);
else
    y(33)=parf4(3)/(parf4(4)+x(32)^parf4(5))-parf4(6)*x(33);
end
y(34)=parf4(9)+parf4(7)*x(33)-parf4(8)*x(34);
